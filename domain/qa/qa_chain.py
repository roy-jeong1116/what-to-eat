...
def recipe_by_inventory(ingredients: str, user_request: str) -> str:
    prompt = PromptTemplate.from_template("""
        ...
        - 반드시 위 재료에 있는 재료만 사용하세요. 사용하지 않은 재료가 있어도 괜찮습니다.
        - 새로운 재료나 추가 재료는 절대 포함하지 마세요.
        - 레시피는 일반적인 가정식 또는 간단한 요리여야 하며, 너무 특이하거나 복잡한 요리는 피해주세요.
...
# 5. 앱 설명 에이전트
def explain_app() -> str:
    return """안녕하세요! 저는 『뭐먹을냉?』이라는 요리 도우미예요 🍳
냉장고 속 재료와 당신의 요청을 분석해서 알맞은 1인분 요리를 3가지 추천해드려요.
간단히 재료만 입력하셔도 되고, 파스타, 김치찌개 같은 요리명을 주셔도 돼요!
"""    
